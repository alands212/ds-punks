...
contract DsPunksDna {
    ...
    //TODO: replace DterministicPseudoRamdomDna with chain.link to consume random data in production
    
    function DterministicPseudoRamdomDna(uint256 _tokenId, address _minter)
        public
        pure
        returns(uint256)
    {
        uint256 conbineParams = _tokenId + uint160(_minter);
        bytes memory encodedParams = abi.encodePacked(conbineParams);
        ...
        returns (string memory)
    {
        ...
        string memory jsonURI = Base64.encode(
            abi.encodePacked(
                '{ "name": "DsPunks #',
                tokenId.toString(),
                '", "description": "DsPunks is a test NFT developed by Alan Martin Da Silva for more information visit dapps.dasilva.ar", "image": "',
                image,
                '"}'
            )
        );
